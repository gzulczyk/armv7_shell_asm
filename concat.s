...
@ --- remove any newlines and replace it with null-terminator flag --- 
strip_newline:
...
cmd_done:
    ...
    str r2, [r3], #4    @ store the r2 value at memory address which r3 points to and advance it by 4
...
parse_args:
    ...
    str r1, [r3], #4    @ store the r1 value at memory address which r3 points to and adance it by 4
...
terminate_token:
    ...
    strb r4, [r1, #-1]  @ store the value in r4 at address (r1 - 1)
    b parse_args        @ go to the `parse_args` function

@ --- null-terminate args ---
end_args:
   mov r4, #0           @ set 0x00 value into r4 register
   str r4, [r3]         @ store the r4 value inside r3 pointed memory address
